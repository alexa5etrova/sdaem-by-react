...
const NewsItem = (props: NewsItemProps): JSX.Element => {
  ...
  if (status === "resolved") {
    let showenNews = news.find((item) => item.id === newsId);
    let crumbs = [...CRUMBS, { title: showenNews!.title, url: `/news/${showenNews!.id}`, id: 202 }];
    let index = news.findIndex((item) => item.id === newsId);
